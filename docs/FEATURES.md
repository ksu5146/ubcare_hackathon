...
>
> **목표 UX 흐름**:
> 1. 검색 결과 리스트/지도에서 Ctrl+클릭(모바일: long-press)으로 비교 대상 추가 (최대 4개) — **구현 완료**
> 2. 하단 플로팅 바에 선택된 단지 수 표시 + "비교 시작" CTA — **구현 완료**
...
| F6-2a | 비교함 (Compare Cart) | P0 | ✅ | `search/page.tsx` (selectedApts) | Ctrl+클릭(모바일: long-press)으로 단지 선택, 최대 4개, Set 기반 상태 관리 |
| F6-2b | 비교 플로팅 바 | P0 | ✅ | `search/page.tsx` (하단 바) | 하단 플로팅 바에 "N개 단지 선택됨" + "비교 시작" CTA 버튼 구현 |
...
> - ~~미구현 P0: F6-2a 비교함(Compare Cart), F6-2b 플로팅 바~~ → v2.4에서 구현 완료 (Ctrl+클릭 + 하단 비교 바)
