...
> 아키텍처 피벗으로 추가된 기능. 실시간 API 호출 대신 사전 수집된 SQLite DB를 사용한다.

| ID | 기능 | 우선순위 | 상태 | 구현 파일 | 비고 |
|----|------|----------|------|-----------|------|
| F0-1 | SQLite 스키마 초기화 | P0 | ✅ | `lib/db.ts` | trades + complexes + trade_complex_map 테이블, WAL 모드 |
...
| F0-9 | 증분 업데이트 | P1 | 🔲 | - | 최신 1개월 재수집 자동화 미구현 |
...
| F1-2 | 지역 선택 (3단계 캐스케이드) | P0 | ✅ | `components/filter/RegionSelect.tsx` | 시/도 → 시/군/구 → 읍/면/동, `data/region-codes.json` 기반 |
...
| F1-8 | 조건 프리셋 저장 | P2 | ✅ | `components/filter/FilterBookmarks.tsx`, `hooks/use-filter-bookmarks.ts` | 필터 북마크 저장/불러오기, localStorage 기반 |

### F1-B: 확장 필터 (v2.0 신규 — DB 기반, v2.3 전면 구현)
...
| F1-14 | 분양형태 필터 | P2 | 🔲 | - | 분양/임대, complexes.sale_type |
...
| ID | 기능 | 우선순위 | 상태 | 구현 파일 | 비고 |
|----|------|----------|------|-----------|------|
| F2-1 | 지도 뷰 (카카오맵) | P0 | ✅ | `components/map/KakaoMap.tsx` | 마커, 가격 오버레이, DB 좌표(lat/lng) 직접 사용, 뷰포트 기반 성능 최적화, 즐겨찾기 마커 |
| F2-2 | 리스트 뷰 | P0 | ✅ | `components/complex/ComplexCard.tsx`, `ComplexList.tsx` | 카드형, 단지명/가격/면적/년도 |
| F2-3 | 지도-리스트 연동 | P0 | ✅ | `app/search/page.tsx` | 양방향 하이라이트 동기화 |
| F2-4 | 검색 결과 정렬 | P1 | ✅ | `components/filter/SortSelect.tsx` | 가격/면적/건축년도/세대수 정렬 |
| F2-5 | 페이지네이션 | P1 | 🔲 | - | 미구현 |
| F2-6 | 검색 결과 요약 | P1 | 🔲 | - | "총 XX개 단지, 평균 거래가 X억" 표시 |
...
> **참고**: 호재/학군 탭은 실데이터 연동 전 mock 데이터 기반이었으나, 품질 문제로 단지 상세 페이지에서 **비활성화** 처리됨. 실데이터(서울열린데이터, NEIS) 연동 완료 시 재활성화 예정.
...
| F5-2 | 대중교통 출퇴근 소요시간 | P0 | ✅ | `components/transit/CommuteResult.tsx` | ODsay API, 환승횟수/요금 |
| F5-5 | 3대 업무지구 접근성 | P1 | ✅ | `components/transit/BusinessDistrictCommute.tsx` | 강남/여의도/광화문 소요시간, ODsay API, DB 좌표(lat/lng) 직접 전달 방식 (카카오 지오코딩 의존성 제거) |
| F5-3 | 자차 출퇴근 | P1 | 🔲 | - | 미구현 |
...
>
> **현재 구현**: 나의관심단지에서 2~4개 단지 선택 → JSON URL 파라미터로 비교 페이지 이동 → DB 직접 조회로 실거래가 오버레이 차트 + 비교 테이블 표시. AI 인사이트(Azure OpenAI) 연동. `useSyncExternalStore` 기반 실시간 동기화.
>
> **목표 UX 흐름**:
...
| F6-2 | 단지 비교 뷰 (2~4개) | P0 | ✅ | `app/compare/page.tsx`, `components/chart/CompareChart.tsx` | JSON URL 파라미터, DB 직접 조회, N개 단지 오버레이 차트 + 비교 테이블 |
...
| F6-3 | 비교분석 히스토리 | P0 | ✅ | `hooks/use-comparisons.ts`, `app/api/user/comparisons/route.ts` | 자동 저장 (최대 10개), 동일 조합 중복 방지, 클릭 시 복원 |
| F6-4 | 비교분석 즐겨찾기 | P0 | ✅ | `hooks/use-comparisons.ts`, `app/compare/page.tsx` | 히스토리→즐겨찾기 전환, 직접 저장 (최대 10개) |
| F6-5 | 비교 항목 확장 (메타데이터) | P1 | 🔲 | - | 세대수, 주차대수, 난방방식, 건축년도, 시공사 등 |
...
| F7-4 | 관심단지 DB 저장 | P0 | ✅ | `app/api/user/favorites/route.ts`, `hooks/use-favorites.ts` | 로그인 시 DB, 비로그인 시 localStorage |
| F7-5 | 관심단지 로그인 동기화 | P0 | ✅ | `app/api/user/favorites/sync/route.ts` | 로그인 시 로컬→DB 병합 |
| F7-6 | 비교분석 DB 저장 | P0 | ✅ | `app/api/user/comparisons/route.ts`, `hooks/use-comparisons.ts` | 히스토리/즐겨찾기 구분, 로그인 시 DB |
| F7-7 | 필터 프리셋 저장 | P2 | 🔲 | `app/api/user/filters/route.ts` | user_filters 테이블 준비 완료, UI 미구현 |
| F7-8 | 나의관심단지 전체 보기 | P1 | 🔲 | - | 네비바에 전체 관심단지 지도 표시 페이지 |

---

## F8: 단지 상세 — 건물지표 및 단지 평가 (v2.3 신규)

> 단지 상세 페이지에서 건물 지표(용적률/건폐율/에너지등급)와 AI 기반 단지 평가(재건축가능성/주거쾌적성/미래가치)를 InfoTooltip과 함께 표시한다.

| ID | 기능 | 우선순위 | 상태 | 구현 파일 | 비고 |
|----|------|----------|------|-----------|------|
| F8-1 | 건물지표 표시 | P1 | ✅ | `components/complex/ComplexInfoCard.tsx` | 용적률/건폐율/에너지등급 + InfoTooltip |
| F8-2 | 단지 평가 스코어 표시 | P1 | ✅ | `components/complex/ComplexInfoCard.tsx` | 재건축가능성/주거쾌적성/미래가치, `lib/collector/scoring.ts` 계산값 |
...
| ID | 기능 | 우선순위 | 상태 | 구현 파일 | 비고 |
|----|------|----------|------|-----------|------|
| F9-1 | AI 단지 비교 분석 | P1 | ✅ | `components/compare/AiInsight.tsx`, `app/api/ai/insight/route.ts` | Azure OpenAI, 스코어링 데이터(재건축가능성/주거쾌적성/미래가치) 포함 |
| F9-2 | 출퇴근 비교 패널 | P1 | ✅ | `components/compare/CommuteCompare.tsx` | 비교 단지 간 출퇴근 소요시간 비교 |
...
> v2.3 기준: F1 확장 필터(건축년도/세대수/층수/고급필터) 전면 구현 완료. F2 정렬 기능 추가. F4 주변분석 탭 비활성화(mock 데이터). F5 3대 업무지구 접근성(BusinessDistrictCommute) 추가. F6 비교 페이지에 AI 인사이트 연동. F8 단지 상세 건물지표·평가 스코어 추가. F9 AI 인사이트·출퇴근 비교 추가. 미구현 P0는 F6-2a 비교함, F6-2b 플로팅 바.
